...
Phases 0–2: `\context(\writing)` (**H-T**, write confinement), `\context(\reading)` (**H-I1**, read
confinement), and `\context(\postcond)` (**H-R**, audit-log completeness + append-only). Builds and is
verified against **Frama-C 32.1 (Germanium)**; `./tests/run.sh` is green (14/14). The roadmap to full
STRIDE coverage is in [`happy-roadmap.md`](happy-roadmap.md); the design rationale is
[`macsl-impl.md`](macsl-impl.md).
